...
export default function BulletinBoard({ bulletin, lastEditedBy, lastEditedAt, allMembers, onUpdate }) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(bulletin || "");
  const textareaRef = useRef(null);

  useEffect(() => {
    if (!editing) setDraft(bulletin || "");
  }, [bulletin, editing]);
      ...
      {editing ? (
        ...
        <div>
          <div
            ...
            style={{
              fontSize: 13,
              color: bulletin ? colors.textMuted : colors.textGhost,
              lineHeight: 1.75,
              whiteSpace: "pre-wrap",
              cursor: "text",
              minHeight: 60,
          ...
          >
            {bulletin || "Click to post something..."}
          </div>
